...
async def file_move(params: FileMoveParams) -> FileMoveResult:
    ...
    Raises:
            SegError: Raised with stable action error codes on security,
    ...
    if source_ext != destination_ext:
        raise SegError(CONFLICT, "File extension change is not allowed.")
    ...
    try:
        ...
        if not params.overwrite:
            raise SegError(CONFLICT) from exc
        ...
        raise SegError(CONFLICT, str(exc)) from exc
    except PathSecurityError as exc:
        raise SegError(PATH_NOT_ALLOWED, str(exc)) from exc
    ...
    try:
        ...
        raise SegError(PERMISSION_DENIED) from exc
